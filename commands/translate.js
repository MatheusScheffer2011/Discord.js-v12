...
let langs = {
 ...
 exports.run = (client, message, args) => {
  ...
  if (!args[0]) {
    return message.channel.send('Para traduzir use a assim m!translate [**Linguagem inicial do texto**] [**Linguagem para qual voce quer que eu traduza**] [**Texto**]\n\n**Lembrete use só as iniciais.**\npor exemplo:m!translate in(inglês) pt(português) Hi my friend.')
  }
  ...
  translate(msg, { from: args[0], to: args[1] }).then(res => {
    ...
    message.channel.send('**Está errado a forma que você digitou. Tente de novo!**')
  })
};
